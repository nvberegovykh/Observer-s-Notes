...
reader_guide = '''## How to read this volume
...
'''
if '## How to read this volume' not in book:
    anchor = '## A note before sailing\n'
    assert anchor in book
    book = book.replace(anchor, reader_guide + anchor, 1)

audit = '''# Critical audit · Where Compass can be wrong
...
'''
if '# Critical audit · Where Compass can be wrong' not in book:
    anchor = '# Working definitions added in Vol. 1.2\n'
    assert anchor in book
    book = book.replace(anchor, audit + anchor, 1)

ref_explainer = '''## How the references relate to the ideas
...
'''
if '## How the references relate to the ideas' not in book:
    anchor = '# Reference map\n'
    assert anchor in book
    book = book.replace(anchor, ref_explainer + anchor, 1)

map_tail = '- ecosystem conditions and present human well-being: [R28]\n'
if '- scale, hierarchy, emergence, and model-class boundaries: [R29]–[R34]' not in book:
    assert map_tail in book
    book = book.replace(map_tail, map_tail + '- scale, hierarchy, emergence, and model-class boundaries: [R29]–[R34]\n', 1)
...
'''
if '**[R29] Simon, Herbert A.**' not in book:
    anchor = '\n---\n\n## The extension in one passage\n'
    assert anchor in book
    book = book.replace(anchor, '\n' + new_refs.strip() + anchor, 1)

old = '- The model should be no more detailed than the decision requires, but no less detailed than its hard dependencies require.\n'
if 'the local model cannot self-authorize the exception' not in book:
    assert old in book
    add = old + '- If a core constraint is treated as breakable, require an independent verifier with a named relation to that constraint; the local model cannot self-authorize the exception.\n- If persistent evidence looks impossible, challenge spatial, temporal, organizational, and model-class boundaries before inventing causes.\n- Preserve lineage/provenance when a present snapshot cannot explain inherited structure.\n'
    book = book.replace(old, add, 1)

scale_state = '''scale_and_inheritance:
  spatial_scale: ""
  temporal_scale: ""
  organizational_scale: ""
  model_class: ""
  scale_boundary_questioned: false
  scale_question_reason: ""
  related_systems: []
  # - id: "S1"
  #   relation_type: "PART_OF|SUPPORTS|CONTAINS|REGULATES|OBSERVES|AUTHORIZES|INTERACTS"
  #   system: ""
  #   evidence_ids: []
  core_constraint_exception:
    proposed: false
    constraint_ids: []
    reason: ""
    same_local_model_cannot_self_authorize: true
    verifier: ""
    verifier_relation: ""
    verifier_evidence_ids: []
    result: "NOT_REQUIRED|UNRESOLVED|SUPPORTED|REJECTED"
  lineage:
    snapshot_sufficient: null
...
'''
if 'scale_and_inheritance:' not in state:
    anchor = 'navigation:\n'
    assert anchor in state
    state = state.replace(anchor, scale_state + anchor, 1)

check_anchor = '  minimum_system_map_built_before_structural_search: false\n'
if 'core_constraint_exception_has_independent_verification' not in state:
    assert check_anchor in state
    scale_checks = ('  scale_relation_types_declared_when_cross_scale_reasoning_used: false\n'
                    '  core_constraint_exception_has_independent_verification: false\n'
                    '  persistent_impossibility_triggers_scale_or_model_class_review: false\n'
                    '  lineage_preserved_when_snapshot_is_insufficient: false\n')
    state = state.replace(check_anchor, check_anchor + scale_checks, 1)

scale_card = '''## Scale / inheritance check
...
'''
if '## Scale / inheritance check' not in card:
    anchor = '## Patience / branch preservation\n'
    assert anchor in card
    card = card.replace(anchor, scale_card + anchor, 1)

core = '> Preserve the conditions that make viable activity, correction, communication, and generation possible now—not only later.\n'
extra = '> When a conclusion requires suspending a core constraint, require an independent path that can prove the exception wrong.\n'
if extra.strip() not in card:
    assert core in card
    card = card.replace(core, core + extra, 1)

book_path.write_text(book, encoding='utf-8')
state_path.write_text(state, encoding='utf-8')
card_path.write_text(card, encoding='utf-8')

assert '# Critical audit · Where Compass can be wrong' in book
assert '## How the references relate to the ideas' in book
assert '[R34]' in book
assert 'Quantum entanglement is useful here as an extreme boundary example' in book
assert 'same_local_model_cannot_self_authorize: true' in state
assert '## Scale / inheritance check' in card
